...
export default function OrderHistoryList({ limit = 5 }: OrderHistoryListProps) {
    ...
    const listToDisplay = showAll ? allOrders : allOrders.slice(0, limit);
    ...
    if (allOrders.length === 0) return <p className="text-center text-gray-500 mt-4">У вас пока нет заказов.</p>;
    ...
    return (
        <div className="w-full">
                                 ...
                                 {parseError ? (
                                     ...
                                     <ul className="space-y-1.5 text-sm px-2">
                                         {parsedItems.map((item, index) => {
                                             ...
                                             return (
                                                  <li key={`${item.id}-${index}-${item.productItemId}`} className="flex justify-between items-center py-0.5 border-b last:border-b-0">
             ...
             {allOrders.length > limit && (
                 <div className={'mt-4'}>
                     <Button variant="link" onClick={() => setShowAll(!showAll)} className="text-primary p-0 h-auto">
                     {showAll ? 'Свернуть' : `+ Показать все (${allOrders.length})`}
                     </Button>
                 </div>
             )}
        </div>
